...
TURN_VALUE = 0.35 # proportional value of error (PID Control)
...
def action(x, y, w, h, image, cnt):
    ...
    if np.count_nonzero(dst) > 300000:
...
def main():
    ...
    while True:
        try:
            ...
            while True:
                x,y,w,h, image, cnt = myrover.detect_blue_sign(myrover.getImageName())
                ...
                if w*h < 29000:  # Keep moving till the sign is big enough to read the command.
                    ...
                    x,y,w,h, action_image, action_cnt = myrover.detect_blue_sign(myrover.getImageName())
                    gap = myrover.compare_centroids('tmp.jpg',x,y,w,h)
            ...
            current_watch_point = (current_watch_point + 1) % number_of_watch_point
            myrover.turn(165)
            time.sleep(2)         
